...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for i in range(len(alphabet)):
            ...
            while a < 0:
                a += 26  
            cipher[alphabet[i]] = alphabet[a]
            cipher[alphabet[i].upper()] = alphabet[a].upper()
        return cipher
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        most_words = 0
        for s in range(26):
            test_message = self.apply_shift(s)
            words = sum(w.lower() in self.valid_words for w in test_message.split())
            if words > most_words:
                best_shift = s
                most_words = words
